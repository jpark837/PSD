...
class fragmentSNR:
    ...
    def __init__(self, path_to_spectrum, path_to_predicted, residue):
    ...
    def prune_signal(self, array_spectrum, array_fragments):
        ...
        index_b, index_y, index_max = np.empty([1,0], dtype = int), np.empty([1,0], dtype = int), np.empty([1,0], dtype = int) # Initialize empty index array
        for fragment_mz in b_ion:    
            x = self.window_regexmatcher(array_spectrum.astype(int), fragment_mz, 10)
        ...
        index_delete = index_max[index_max!=np.array(None)].astype(int)
